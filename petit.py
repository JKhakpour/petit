...
def text_to_num(s):
    ...
    groups = groupby(
        zip([None] + a, a),
        key=lambda x: x[1] in constants.Magnitude.values()
        and (not (x[0]) or (x[0] < x[1])),
    )
...
def num_to_text(number=0):
    def three_digit(number):
        ...
        if number < 10:
            ...
            result += (
                (
                    constants.TENS[int(number / 10)]
                    + constants.AND
                    + three_digit(number % 10)
                )
                if (number % 10)
                else constants.TENS[int(number / 10)]
            )
        elif number < 1000:
            result += (
                constants.HUNDREDS[int(number / 100)]
                + constants.AND
                + three_digit(number % 100)
                if (number % 100)
                else constants.HUNDREDS[int(number / 100)]
            )
    ...
    if number < 0:
        ...
        result = constants.AND.join(
            [
                (three_digit(digit) + constants.POWERS_OF_THOUSAND[int(order / 3)])
                for digit, order in td
            ]
        )
    return result.replace("  ", " ").strip()
...
def num_to_ordinal(number=0):
    ...
    if number_s in constants.IRREG_ORD_NUMS:
        ...
        match = list(
            filter(
                lambda m: m if m.match(number_s) else None, constants.IRREG_ORD_REGEXES
            )
        )[-1]
        repls = constants.IRREG_ORD_REGEXES[match]
        result = constants.AND.join(
            [num_to_text(match.sub(repl, number_s)) for repl in repls[:-1]]
            + [num_to_ordinal(match.sub(repls[-1], number_s))]
        )
    else:
